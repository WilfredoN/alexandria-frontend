...
export class ScheduleService {
	...
	getSchedules(group_name: string): Observable<Schedule[]> {
		...
		return this.http.get<Schedule[]>(
			`${this.apiURL}/schedule/groups/${group_name}`,
		);
	}

	getLessonById(
		id: number,
	): Observable<{ id: number; subject_name: string }> {
		return this.http.get<{ id: number; subject_name: string }>(
	...
	createLesson(
		...
		return this.http.post<{ id: number; subject_name: string }>(
			`${this.apiURL}/subjects/create`,
			{ subject_name },
		);
	}
